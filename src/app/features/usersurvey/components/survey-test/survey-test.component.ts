...
export class SurveyTestComponent {
  ...
  async getSurveyById(): Promise<any> {
    ...
    try {
      ...
      const userSurveys: any = await this.userSurveyService
        .getSurveys()
        .toPromise();
      if (userSurveys) {
  ...
  checkUndefinedAnswer(): void {
    for (let index = 0; index < this.answerArray.length; index++) {
      if (
        !this.answerArray[index].hasOwnProperty('answer') ||
        this.answerArray[index].answer === undefined
      ) {
  ...
  labelTitle(number: number, sliderOptions: any, item: any): string {
    ...
    if (number == 1 && sliderOptions.length > 1) {
      ...
      sliderOptions.length >= 3 &&
      number === sliderOptions.length / 2 + 0.5
    ) {
  ...
  async saveSurveyAnswers(): Promise<void> {
    ...
    try {
      ...
      if (answers) {
        ...
        if (checkAnswer) {
          const editedAnswer: any = await this.answerService
            .updateAnswer(checkAnswer.id, editBody)
            .toPromise();
          if (editedAnswer) {
          ...
          const createdAnswer: any = await this.answerService
            .createAnswer(createBody)
            .toPromise();
          if (createdAnswer) {
            this.surveyStateInProgress();
  ...
  ChekingRequiredQuestions(): boolean {
    ...
    for (let index = 0; index < this.survey.questions.length; index++) {
      if (this.survey.questions[index].settings.required === true) {
        // Cheking if the asnwer is in the answers Array.
        const question = this.answerArray.find(
          (e: any) => e.questionInfo.id === this.survey.questions[index].id
        );
  ...
  CheckingAnswerByDefect(): void {
    for (let index = 0; index < this.survey.questions.length; index++) {
      if (this.survey.questions[index].settings.answer_value !== '') {
        // Cheking if answer_defect contains one.
        if (this.survey.questions[index].type === 'checkbox') {
          this.setAnswer({
            item: this.survey.questions[index],
            answer: {
              option: this.survey.questions[index].settings.answer_value,
            },
          });
        } else {
          this.setAnswer({
            item: this.survey.questions[index],
            answer: this.survey.questions[index].settings.answer_value,
          });
        }
      }
    }
  ...
  getTableQuestionAnswer(numeral: string, y: number, i: number): boolean {
    for (let index = 0; index < this.answerArray.length; index++) {
      if (
        this.answerArray[index].questionInfo.numeral === numeral &&
        this.answerArray[index].answer !== undefined
      ) {
        if (
          this.answerArray[index].answer.row === i &&
          this.answerArray[index].answer.column === y
        ) {
          return true;
        }
      }
  ...
  verifyVideoLink(item: any, type: string): boolean {
    if (
      (item.settings.question_multimedia === '' && type === 'question') ||
  ...
  veirifyRows(item: any): void {
    ...
    for (let index = 0; index < this.answerArray.length; index++) {
      ...
      if (
        currentAnswer.questionInfo.numeral === item.questionInfo.numeral &&
        currentAnswer.answer !== undefined
      ) {
        if (
          currentAnswer.answer.row === item.answer.row &&
          currentAnswer.answer.column === item.answer.column
        ) {
  ...
  removeRequiredQuestionMessage(item: any): void {
    const checkNumeral = this.questionsToWarn.findIndex(
      (e: any) => e === item.questionInfo.numeral
    );
    if (checkNumeral !== -1) {
      if (item.questionInfo.type === 'table') {
        ...
        for (let index = 0; index < this.answerArray.length; index++) {
          if (
            this.answerArray[index].questionInfo.numeral ===
            item.questionInfo.numeral
          ) {
            count += 1;
          }
        }
