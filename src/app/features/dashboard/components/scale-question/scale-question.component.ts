...
export class ScaleQuestionComponent {
  ...
  saveScaleData(): void {
    ...
    if (storedQuestions) {
      const index = storedQuestions.findIndex(
        (e: any) => e.id === this.scaleForm.value.id
      );
  ...
  onSliderInput(event: Event): void {
    ...
    this.slideValueElement.nativeElement.style.left =
      numberOption === 1 ? numberOption * 0 : numberOption * 10 - 10 + '%';
    this.showValue = true;
  }
  ...
  labelTitle(item: number): string {
    ...
    if (item == 1 && this.sliderOptions.length > 1) {
      ...
      this.sliderOptions.length >= 3 &&
      item === this.sliderOptions.length / 2 + 0.5
    ) {
